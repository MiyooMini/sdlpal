...
static SDL_Surface       *gpScreenDest       = NULL;

//added by cb
static SDL_Surface       *gpScreenTarget     = NULL;
...
--*/
{
...
#else
   //add by cb
   gpScreenTarget = SDL_SetVideoMode(wScreenWidth, wScreenHeight, 32, SDL_HWSURFACE);
   gpScreenDest = SDL_CreateRGBSurface(SDL_HWSURFACE, wScreenWidth, wScreenHeight, 16,  0xF800, 0x7E0, 0x1F, 0);      

   gpScreenReal = SDL_CreateRGBSurface(SDL_SWSURFACE, wScreenWidth, wScreenHeight, 8, 0, 0, 0, 0);
#endif
  ...
  //     gpScreenReal->format->Bmask, gpScreenReal->format->Amask);
   ...
   gpScreen = SDL_CreateRGBSurface(gpScreenReal->flags & ~SDL_HWSURFACE, 320, 200, 8, 0, 0, 0, 0);

   gpScreenBak = SDL_CreateRGBSurface(gpScreenReal->flags & ~SDL_HWSURFACE, 320, 200, 8, 0, 0, 0, 0);
...
--*/
{
   ...
   //added by cb
   if(gpScreenDest != NULL){
      SDL_FreeSurface(gpScreenDest);
   }
   gpScreenDest = NULL;
}
...
--*/
{
...
#else
      //add by cb
      SDL_BlitSurface(gpScreenReal, NULL, gpScreenDest, NULL);
      SDL_BlitSurface(gpScreenDest, NULL, gpScreenTarget, NULL);
      SDL_UpdateRect(gpScreenTarget, dstrect.x, dstrect.y, dstrect.w, dstrect.h);      
      //SDL_UpdateRect(gpScreenDest, dstrect.x, dstrect.y, dstrect.w, dstrect.h);
#endif
   }
   else if (g_wShakeTime != 0)
   {
      ...
      SDL_BlitSurface(gpScreenReal, NULL, gpScreenDest, NULL);
      SDL_BlitSurface(gpScreenDest, NULL, gpScreenTarget, NULL);
      SDL_UpdateRect(gpScreenTarget, 0, 0, gpScreenReal->w, gpScreenReal->h);      
      //SDL_UpdateRect(gpScreenDest, 0, 0, gpScreenReal->w, gpScreenReal->h);
#endif
      g_wShakeTime--;
   }
   else
